    ...
    public class ThirdPersonCharacter : MonoBehaviour {
        ...
        void ScaleCapsuleForCrouching(bool crouch) {
            if (isGrounded && crouch) {
                ...
                body.height = body.height / 2f;
                body.center = body.center / 2f;
                m_Crouching = true;
            } else {
                Ray crouchRay = new Ray(m_Rigidbody.position + Vector3.up * body.radius * k_Half, Vector3.up);
                float crouchRayLength = m_CapsuleHeight - body.radius * k_Half;
                if (Physics.SphereCast(crouchRay, body.radius * k_Half, crouchRayLength, Physics.AllLayers, QueryTriggerInteraction.Ignore)) {
                    m_Crouching = true;
                    return;
                }
